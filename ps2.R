...
print(paste('average number of rolls is:', avg.rolls, 'therefore average number of dice is:', avg.rolls*2))
...
congress_stats<-function(group.var){
  ...
  groups<-unique(congress_age[,group.var]) # get all levels of the group variable
  df<-data.frame('group'=groups, 'avg.age'=NA) # create data frame to store data
  
  for(group in groups){
    # for each level in the group
    group_mask<-congress_age[,group.var] == group # mask to only get entries in the level
